// MCP 服务器爬虫：GitHub topic:mcp-server，结果写入 data/pending/
const fs = require('fs');
...
(async () => {
  try {
    const res = await searchRepos('topic:mcp-server', 30, TOKEN);
    ...
    fs.writeFileSync(out, JSON.stringify({ source: 'github-topic:mcp-server', crawledAt: new Date().toISOString(), items: drafts }, null, 2) + '\n');
