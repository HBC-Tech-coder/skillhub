// WorkBuddy 技能爬虫：GitHub topic 搜索，结果写入 data/pending/
const fs = require('fs');
...
(async () => {
  try {
    const res = await searchRepos('topic:workbuddy OR topic:workbuddy-skills', 30, TOKEN);
    ...
    fs.writeFileSync(out, JSON.stringify({ source: 'github-topics:workbuddy', crawledAt: new Date().toISOString(), items: drafts }, null, 2) + '\n');
