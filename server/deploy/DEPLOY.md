...
2. 编辑 `/etc/systemd/system/skillhub.service`：替换 `SKILLHUB_ADMIN_TOKEN`（64 位随机 hex，AI 维护入口）与可选的 `GITHUB_TOKEN`（推送权限；或给 skillhub 用户配 deploy key）。`systemctl daemon-reload && systemctl restart skillhub`。
